<x-guest-layout>
    ...
    {{-- Google Sign-In --}}
    <div class="mb-6">
        <a href="{{ route('auth.google') }}" class="w-full flex items-center justify-center gap-3 px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-200 transition shadow-sm">
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="mt-4">
            <div class="flex items-center justify-between">
                ...
                @if (Route::has('password.request'))
                    <a class="text-xs font-medium text-teal-600 hover:text-teal-700 transition" href="{{ route('password.request') }}">
                        Forgot password?
                    </a>
                @endif
            </div>
        ...
        <div class="mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox" class="w-4 h-4 rounded border-gray-300 text-teal-600 shadow-sm focus:ring-teal-500 focus:ring-offset-0" name="remember">
        ...
        <p class="mt-6 text-center text-sm text-gray-500 dark:text-gray-400">
            ...
            <a href="{{ route('register') }}" class="font-medium text-teal-600 hover:text-teal-700 transition">
                Create one
            </a>
        </p>
    </form>
